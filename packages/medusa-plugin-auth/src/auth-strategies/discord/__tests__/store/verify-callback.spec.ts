...
import { getDiscordStoreStrategy } from '../../store';

describe('Discord store strategy verify callback', function () {
	...
	let profile: Profile;
	let discordStoreStrategy: IStrategy;
	let updateFn;
	let createFn;
						...
						retrieveRegisteredByEmail: jest.fn().mockImplementation(async (email: string) => {
							...
							if (email === existsEmailWithMetaAndProviderKey) {
								return {
									...
									metadata: {
										[CUSTOMER_METADATA_KEY]: true,
										[AUTH_PROVIDER_KEY]: DISCORD_STORE_STRATEGY_NAME + '_test',
									},
								};
							}
	...
	describe('when strict is set to store', function () {
		beforeEach(() => {
			const DiscordStoreStrategy = getDiscordStoreStrategy('test');
			discordStoreStrategy = new DiscordStoreStrategy(
				container,
				{} as ConfigModule,
				{
		...
		it('should succeed', async () => {
			...
			const data = await discordStoreStrategy.validate(req, accessToken, refreshToken, profile);
			expect(data).toEqual(
		...
		it('should fail when the customer exists without the metadata', async () => {
			...
			const err = await discordStoreStrategy.validate(req, accessToken, refreshToken, profile).catch((err) => err);
